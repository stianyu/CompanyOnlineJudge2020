...
public class CheckDomainTest3 {
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        String str8 = "www--bigo.arg";  // code 3
        ...
        System.out.println("www.bigo.com".getBytes("utf-8").length);
        System.out.println("www.bigo.com".getBytes("unicode").length);
        System.out.println("www.bigo.com".getBytes("gbk").length);

        String string = "OrderService";
        String string2 = string.substring(0,1).toLowerCase() + string.substring(1);
        System.out.println(string2);

        String string3 = "OrderService";
        char[] chars = string3.toCharArray();
        chars[0] = (char) (chars[0] - 'A' + 'a');
        String sting4 = new String(chars);
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (isWhat(chars[i]) > 0) {
                ...
                if (i == 0 || i == chars.length - 1) {
                    ...
                    if (isWhat(chars[i]) == 3) {
                        // 如果当前字符为连接符，且上一个字符为连接符或分隔符，则输出错误代码3
                        if (lastCharIsLink || lastCharIsSplit) {
                            code = 3;
                            break;
                        } else {
